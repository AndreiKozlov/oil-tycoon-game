...
Пользователь утром проверит карту визуально в браузере. Если ок — начнём **G.4** (Postgres + Fastify + Telegram auth + Natural Earth raster для реальной географии).

Параллельно пользователь рисует UI-макеты.
