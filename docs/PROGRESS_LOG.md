...
### 2026-05-18 — G.3.2 Удалены Tshrc/Tshre + аудит тайлов + прод-размер ✅

После замечаний пользователя:
- **Удалены 38 файлов** `Tshrc*` + `Tshre*` из `bg/` — несовместимые стыки.
- Wang-таблица берегов сократилась до 4 базовых watrtl-кодов.
- Аудит сухопутных тайлов по среднему цвету:
  - `tilemap/tgrb` (#30491d ярко-зелёная трава, 24 файла) — настоящая зелень, оказалось правильнее для grassland.
  - `tilemap/tdtb` (#6b4d2e обычная земля) — для plain.
  - `bg/tswb` (#2a4e2f тёмная зелень) — для forest.
  - `bg/trom` + `tilemap/rocktl` (всего 11+) — для mountain.
- Прод-размер карты: **452×227 = 102 604 тайла**, цель 30 000 сухопутных.
- MIN_ZOOM = 0.1 (вид с орбиты), стартовый zoom авто-fit.
- **Вода неактивна** — клик игнорируется, hover серая полупрозрачная рамка.

### 2026-05-18 — G.3.3 Визуальный аудит карты и фиксы ✅

Серверный рендер всей карты через Python+PIL (зеркало TypeScript). Найдены и исправлены проблемы:

**Гор почти не было (135 тайлов = 0.13%)** → ridge-noise (отдельный fbm) для горных хребтов. Теперь `elev > 0.35 && ridge > 0.5` ИЛИ `elev > 0.55` → **3.24%** гор.

**Лес не вырастал на экваторе** — пустыня забивала. Фикс: разделил экватор (moisture > 0.45 → лес = Амазония) и субтропики (сухость → пустыня).

**Wang-стыки берегов — 4 из 14** → добавлены **трансформации** flipH/flipV/rotate180 к базовым watrtl. Все 15 wang-кодов покрыты из 4 натуральных через зеркала. В Canvas: `ctx.translate + scale(-1,1)`.

**Морской уровень 0.42 → 0.56** = ровно 30 023 сухопутных тайла (цель GDD), 70.7% воды как Земля.

### Финальные цифры карты

```
desert         2639 ( 2.57%)
forest         8235 ( 8.03%)
grassland     10106 ( 9.85%)
mountain       3323 ( 3.24%)
plain          1015 ( 0.99%)
swamp           620 ( 0.60%)
tundra         4085 ( 3.98%)
water         72581 (70.74%)
────────────────────────────
сухопутных:   30023 (29.3%)
```

Preview: `docs/map-preview-v3.png` (1600×804).

---

### 2026-05-18 — G.3.4 Jitter биом-границ ✅

Добавлен мелкий value-noise jitter (±0.04) к elevation и moisture. Эффект: границы биомов стали зубчатыми с взаимными вкраплениями. Не настоящий wang, но визуально лучше чем прямые линии.

### 2026-05-18 — G.3.5 Попытка полного wang между биомами — НЕ УДАЛАСЬ ❌

Гипотеза: префиксы `tgrd/tgrm/tsnd/...` в Civ-II tileset — это wang-transitions. После пиксельного аудита всех 20 файлов `tgrd*` обнаружено: они **все классифицируются как LLLLL (чистая трава)**. Это **декоративные варианты base-биома**, не transitions.

**Вывод:** текущий набор тайлов **не содержит** настоящих wang-переходов между биомами суши. Civ-II original использовал другую систему (overlays/edge bitmasks в отдельных файлах).

### 2026-05-18 — G.3.6 Промпты для нейросети ✅

Документ `docs/AI_TILESET_PROMPT.md` — 3 промпта (Midjourney/SDXL/DALL-E) + workflow для генерации tilesets + предупреждения о pixel-perfect ограничениях нейросетей + альтернативы (itch.io, Fiverr, Sprite Fusion).

### 2026-05-18 — G.3.7 Процедурный рендер БЕЗ PNG ✅ ⭐

**Решающее решение**: отказались от внешних тайлов вообще. Карта рисуется **полностью в коде** через Canvas ImageData.

**`apps/web/src/lib/proceduralTiles.ts`**:
- 9 палитр биомов: main + dark + light + accent (декорация).
- Функция `renderTilePixels(biome, x, y, neighbors)` — создаёт ImageData 32×32 пиксели:
  - **Bayer 4×4 dither pattern** для wang-переходов на границах.
  - **8-пиксельная blend-зона** у каждой стороны тайла — постепенное смешивание с соседним биомом.
  - **Per-pixel value noise** + ±12 jitter для текстуры биома.
  - **Decoration points** — точки accent-цвета (цветы, снег, лава) с разной плотностью на биом.
- Кэш `tileCache: Map<string, ImageData>` — переиспользует одинаковые конфигурации соседей (4 варианта вращения).

**`WorldMapCanvas.tsx` переписан**:
- Убрана загрузка PNG, sprite cache.
- Off-screen canvas 32×32 для рендера одного тайла → `drawImage` на основной с масштабом.
- Wang-логика для воды убрана (теперь Bayer-dither работает для всех биомов включая воду↔сушу).

**Результат — стиль Polytopia / Risk**: плоский, читаемый, идеальные стыки. Без PNG = нет проблем с лицензией, размером ассетов, выравниванием пикселей.

**Биомы (preview procedural):**
```
desert: 1902 (1.9%) | forest: 9519 (9.3%) | grassland: 9658 (9.4%)
mountain: 2215 (2.2%) | plain: 527 (0.5%) | swamp: 1093 (1.1%)
tundra: 7290 (7.1%) | water: 70400 (68.6%)
```

Preview: `docs/map-preview-procedural.png` (2.3 MB вся карта), `docs/map-zoom-procedural.png` (zoom на стыки).

**Удалено 140 файлов** из `tiles/raw/`: EDG/TRDC/rocktl/icyrvr/lavrvr/mudrvr (пользователь просил, и они больше не нужны для процедурного рендера).

---

## Следующий шаг

Пользователь проверит процедурную карту визуально. Если ок — идём в **G.4** (Postgres + Fastify + Telegram auth + перенос карты на серверный state).
