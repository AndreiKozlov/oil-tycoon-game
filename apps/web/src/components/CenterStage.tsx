...
export function CenterStage({ buildings }: Props) {
  return (
    <div className="relative flex-1 overflow-hidden bg-gradient-to-b from-slate-900 via-slate-950 to-black">
      ...
      >
        ...
        {/* Вышка (derrick) — каркас + флагшток */}
        <g transform="translate(140, 130)">
          <polygon
            points="0,40 30,40 25,0 5,0"
            fill="none"
            stroke="#f59e0b"
            strokeWidth="1.5"
          />
        ...
        {/* Резервуар (tank) — заполненный цилиндр */}
        ...
        {/* Индикаторы статуса над постройками — точки */}
        {buildings.map((b, i) => {
            ...
            b.status === 'ok'
              ...
              : b.status === 'needs_repair'
                ...
                : b.status === 'full'
                  ? '#eab308'
                  : '#94a3b8';
          return <circle key={b.id} cx={x} cy={y} r="3" fill={color} />;
        })}
      </svg>
      ...
      <div className="pointer-events-none absolute bottom-2 left-1/2 -translate-x-1/2 rounded-md bg-slate-900/70 px-2 py-1 text-[10px] text-slate-500">
        SVG-заглушка · Pixi.js появится на этапе C
      </div>
    </div>
  );
}
